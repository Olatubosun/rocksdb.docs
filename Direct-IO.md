...
5. It is advisable to turn on mid-point insertion strategy if your workload is a mix of point and range queries, by setting `LRUCacheOptions.high_pri_pool_ratio = 0.5`. (Note that this depends on `BlockBasedTableOptions.cache_index_and_filter_blocks` and `cache_index_and_filter_blocks_with_high_priority` as well).