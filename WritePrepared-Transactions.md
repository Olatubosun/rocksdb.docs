...
We keep track of smallest uncommitted data and store it in the snapshot. When reading the data if its sequence number lower than the smallest uncommitted data, then we skip lookup into CommitCache to reduce cpu cache misses. To be able to correctly maintain what is the smallest uncommitted data, we used the _PreparedHeap_ structure and make the transactions to add to this heap in order so that if the heap top will always increase over time. To ensure in order call to `AddPrepared` we do that in the PreReleaseCallback of the primary write queue.
